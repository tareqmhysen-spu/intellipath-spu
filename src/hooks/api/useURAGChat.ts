/**
 * URAG Chat Hook - Unified RAG Architecture
 * Features: FAQ Layer, Semantic Caching, Memory, Security Guardrails
 */
import { useState, useCallback, useRef } from 'react';
import { useToast } from '@/hooks/use-toast';
import { useAuthStore } from '@/stores/authStore';

interface Message {
  id: string;
  role: 'user' | 'assistant' | 'system';
  content: string;
  sources?: Source[];
  mode?: string;
  confidence?: number;
  timestamp: Date;
}

interface Source {
  code: string;
  name: string;
  department: string;
  score: number;
}

interface StudentContext {
  gpa?: number;
  department?: string;
  year_level?: number;
  credits_completed?: number;
  major?: string;
}

interface URAGOptions {
  studentContext?: StudentContext;
  conversationId?: string;
  useHybridSearch?: boolean;
  useMetadataFilter?: boolean;
}

const URAG_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/urag-query`;

export function useURAGChat() {
  const [messages, setMessages] = useState<Message[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isStreaming, setIsStreaming] = useState(false);
  const [currentMode, setCurrentMode] = useState<string>('');
  const abortControllerRef = useRef<AbortController | null>(null);
  const { toast } = useToast();
  const { user } = useAuthStore();

  const sendMessage = useCallback(async (
    input: string,
    options?: URAGOptions
  ) => {
    // Add user message
    const userMessage: Message = {
      id: Date.now().toString(),
      role: 'user',
      content: input,
      timestamp: new Date(),
    };
    setMessages(prev => [...prev, userMessage]);
    setIsLoading(true);
    setIsStreaming(true);
    setCurrentMode('');

    // Create abort controller
    abortControllerRef.current = new AbortController();

    try {
      const response = await fetch(URAG_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}`,
        },
        body: JSON.stringify({
          question: input, // urag-query expects 'question' not 'messages'
          student_context: options?.studentContext,
          conversation_id: options?.conversationId,
          top_k: 5,
        }),
        signal: abortControllerRef.current.signal,
      });

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        
        if (response.status === 429) {
          toast({
            title: 'تم تجاوز الحد المسموح',
            description: 'يرجى المحاولة لاحقاً',
            variant: 'destructive',
          });
          throw new Error('Rate limit exceeded');
        }
        
        if (response.status === 402) {
          toast({
            title: 'رصيد غير كافٍ',
            description: 'يرجى إضافة رصيد لاستخدام الذكاء الاصطناعي',
            variant: 'destructive',
          });
          throw new Error('Payment required');
        }

        throw new Error(errorData.error || 'Failed to send message');
      }

      if (!response.body) {
        throw new Error('No response body');
      }

      const reader = response.body.getReader();
      const decoder = new TextDecoder();
      let assistantContent = '';
      let sources: Source[] = [];
      let mode = '';
      let confidence = 0;
      const assistantMessageId = (Date.now() + 1).toString();

      while (true) {
        const { done, value } = await reader.read();
        if (done) break;

        const chunk = decoder.decode(value, { stream: true });
        const lines = chunk.split('\n');

        for (const line of lines) {
          if (line.startsWith('data: ') && line !== 'data: [DONE]') {
            const jsonStr = line.slice(6).trim();
            if (!jsonStr) continue;

            try {
              const data = JSON.parse(jsonStr);

              // Handle mode info
              if (data.mode) {
                mode = data.mode;
                setCurrentMode(mode);
              }

              // Handle confidence
              if (data.confidence) {
                confidence = data.confidence;
              }

              // Handle sources
              if (data.sources) {
                sources = data.sources;
              }

              // Handle content
              if (data.content) {
                assistantContent += data.content;
                
                // Update assistant message
                setMessages(prev => {
                  const existing = prev.find(m => m.id === assistantMessageId);
                  if (existing) {
                    return prev.map(m =>
                      m.id === assistantMessageId
                        ? { ...m, content: assistantContent, sources, mode, confidence }
                        : m
                    );
                  }
                  return [
                    ...prev,
                    {
                      id: assistantMessageId,
                      role: 'assistant',
                      content: assistantContent,
                      sources,
                      mode,
                      confidence,
                      timestamp: new Date(),
                    },
                  ];
                });
              }

              // Handle standard OpenAI format
              if (data.choices?.[0]?.delta?.content) {
                assistantContent += data.choices[0].delta.content;
                
                setMessages(prev => {
                  const existing = prev.find(m => m.id === assistantMessageId);
                  if (existing) {
                    return prev.map(m =>
                      m.id === assistantMessageId
                        ? { ...m, content: assistantContent }
                        : m
                    );
                  }
                  return [
                    ...prev,
                    {
                      id: assistantMessageId,
                      role: 'assistant',
                      content: assistantContent,
                      timestamp: new Date(),
                    },
                  ];
                });
              }
            } catch (e) {
              // Ignore parse errors
            }
          }
        }
      }
    } catch (error: any) {
      if (error.name === 'AbortError') {
        console.log('Request aborted');
      } else {
        console.error('URAG error:', error);
        toast({
          title: 'خطأ',
          description: error.message || 'حدث خطأ أثناء الإرسال',
          variant: 'destructive',
        });
      }
    } finally {
      setIsLoading(false);
      setIsStreaming(false);
      abortControllerRef.current = null;
    }
  }, [messages, toast]);

  const stopGeneration = useCallback(() => {
    if (abortControllerRef.current) {
      abortControllerRef.current.abort();
      setIsStreaming(false);
      setIsLoading(false);
    }
  }, []);

  const clearMessages = useCallback(() => {
    setMessages([]);
    setCurrentMode('');
  }, []);

  const setInitialMessages = useCallback((msgs: Message[]) => {
    setMessages(msgs);
  }, []);

  return {
    messages,
    isLoading,
    isStreaming,
    currentMode,
    sendMessage,
    stopGeneration,
    clearMessages,
    setInitialMessages,
  };
}
